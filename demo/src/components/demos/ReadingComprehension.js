...
const Attack = ({requestData,passage_question_attention, question_tokens, passage_tokens, attackData,attackData2, attackModel,attackModel2}) => {  
  ...
  if (attackData === undefined) {
    ...
    let [first,second] = postprocessInputReduction(attackData["original"],attackData["final"][0])    
  ...
  if (attackData2 === undefined) {
    ...
    let [first,second] = postprocessHotflip(attackData2["original"],attackData2["final"][0])
    attack_visual2 = second
    attack_visual2_og = first 
    new_prediction = <p><b>Prediction changed to:</b> {attackData2['new_prediction']}</p>
  }  
  return(
   ...
   <AccordionItem expanded={true}>
          ...
          <AccordionItemBody>                      
            <p> <a href="https://arxiv.org/abs/1712.06751" target="_blank" rel="noopener noreferrer">HotFlip</a> flips words in the question to change the model's prediction. We iteratively flip the word with the highest gradient until the prediction changes.</p>                                
...
const generateInterpretation = (requestData, 
                                ...
                                sgPassageTokensWithWeights,
                                sgQuestionTokensWithWeights,
                                integratedGradients,
                                simpleGradients,
                                smoothGradients,
                                handleTopKChange) => {
    ...
    <OutputField>
      <Accordion accordion={false}>
        ...
        <AccordionItem expanded={true}>
          ...
          <AccordionItemBody>
            <p> See saliency map interpretations generated by <a href="https://arxiv.org/abs/1312.6034" target="_blank" rel="noopener noreferrer">visualizing the gradient</a>. </p>
            ...
            { gradientPassageTokensWithWeights.length !== 0 ? <TextSaliencyMap tokensWithWeights={gradientPassageTokensWithWeights} colormapProps={{colormap: 'copper',
                                                                                        format: 'hex',
                                                                                        nshades: 20}} /> : <p style={{color: "#7c7c7c"}}>Press "interpret prediction" to show the passage interpretation.</p> }            
            <p><strong>Saliency Map for Question:</strong></p>
            { gradientQuestionTokensWithWeights.length !== 0 ? <TextSaliencyMap tokensWithWeights={gradientQuestionTokensWithWeights} colormapProps={{colormap: 'copper',
                                                                                              format: 'hex',
                                                                                              nshades: 20}} /> : <p style={{color: "#7c7c7c"}}>Press "interpret prediction" to show the question interpretation.</p> }
            <button
              type="button"
              className="btn"
              style={{margin: "30px 0px"}}
        ...
        <AccordionItem expanded={false}>
          <AccordionItemTitle>
            ...
            <AccordionItemBody>
            <p> See saliency map interpretations generated using <a href="https://arxiv.org/abs/1703.01365" target="_blank" rel="noopener noreferrer">Integrated Gradients</a>. </p>
            <p><strong>Saliency Map for Passage:</strong></p>
        ...
        {/* ****************************** */}
        {/* SMOOTH GRADIENTS COMPONENT */}
        {/* ****************************** */}
        <AccordionItem expanded={false}>
          <AccordionItemTitle>
              SmoothGrad Interpretation Visualization
              <div className="accordion__arrow" role="presentation"/>
            </AccordionItemTitle>
            <AccordionItemBody>
            <p>See saliency map interpretations generated using <a href="https://arxiv.org/abs/1706.03825" target="_blank" rel="noopener noreferrer">SmoothGrad</a>.</p>
            <p><strong>Saliency Map for Passage:</strong></p>
            { sgPassageTokensWithWeights.length !== 0 ? <TextSaliencyMap tokensWithWeights={sgPassageTokensWithWeights} colormapProps={{colormap: 'copper',
                                                                                        ...
                                                                                        nshades: 20}} /> : <p style={{color: "#7c7c7c"}}>Press "interpret prediction" to show the passage interpretation.</p> }
            <p><strong>Saliency Map for Question:</strong></p>
            { sgQuestionTokensWithWeights.length !== 0 ? <TextSaliencyMap tokensWithWeights={sgQuestionTokensWithWeights} colormapProps={{colormap: 'copper',
                                                                                              format: 'hex',
                                                                                              nshades: 20}} /> : <p style={{color: "#7c7c7c"}}>Press "interpret prediction" to show the question interpretation.</p> }
            <button
              type="button"
...
const AnswerByType = ({requestData, responseData, interpretModel, interpretData, integratedGradients, simpleGradients, smoothGradients, handleTopKChange, attackData, attackData2,attackModel,attackModel2}) => {
  if(requestData && responseData) {
    ...
    let sgQuestionTokensWithWeights = []
    let sgPassageTokensWithWeights = []
    ...
    if (smooth_gradient_interpreter) {
      ...
      let tokensWithWeights = getTokenWeightPairs(grad_input_2, grad_input_1, question_tokens, passage_tokens)
      sgQuestionTokensWithWeights = tokensWithWeights[0]
      sgPassageTokensWithWeights = tokensWithWeights[1]
    }
